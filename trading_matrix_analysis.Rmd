...
Similarly to the production data country names have changed. We are unable to perform similar imputation to the production data. We will therefore perform a more limited analysis of the data for these countries.
...
```
The data is produced from both the exporting and importing countries submissions to the FAO. This has led to discrepencies in the data where one party doesn't report any trading activity between the states or the amounts of chickens traded are very different. We have presented the scale of these discrepencies below. We have used the mean of the reported number of heads (1000 heads) where both countries have reported values. When one of the countries has reported data but the other has not we have used the value reported.
``` {r updated_edgelist}
...
detailed_trading <- as.data.frame(detailed_trading) %>%
  mutate(year = as.numeric(Year),
         Value = as.numeric(Value),
         Element = ifelse(Element %in% c("Import Quantity", "Import Value"), "Import", "Export")) %>%
  select(`Reporter Country Code`, `Partner Country Code`, Element, year, Unit, Value) %>%
  mutate(sender_iso3 = ifelse(Element == "Export",`Reporter Country Code`, `Partner Country Code`),
         receiver_iso3 = ifelse(Element == "Import", `Reporter Country Code`, `Partner Country Code`)) %>%
  mutate(Unit = recode(Unit, "1000 Head" = "head", "1000 US$" = "usd"),
         element = recode(Element, "Import" = "import", "Export" = "export"),
         trade = paste0(element, "_", Unit)) %>%
  select(sender_iso3, receiver_iso3, year, trade, Value) %>%
  pivot_wider(., names_from = trade, values_from = Value) %>%
  mutate_at(c(4:9), ~replace(., is.na(.), 0)) %>%
  select(-c(import_tonnes, export_tonnes))  %>%
  rename("reported_import_head" = import_head,
         "reported_import_usd" = import_usd,
         "reported_export_head" = export_head,
         "reported_export_usd" = export_usd) %>%
  mutate(difference_head = reported_export_head-reported_import_head,
         difference_usd = reported_export_usd-reported_import_usd) %>%
  mutate(import = ifelse(reported_import_head > 0, reported_import_head,
                         ifelse(reported_import_usd > 0, 1, 0)),
         export = ifelse(reported_export_head > 0, reported_export_head,
                         ifelse(reported_export_usd > 0, 1, 0)),
         greatest = ifelse(import >= export, import, export),
         mean = ceiling(rowMeans(data.frame(ifelse(import == 0,
                                                   export,
                                                   import),
                                            ifelse(export == 0,
                                                   import,
...
edgelist <- detailed_trading %>%
  select(-c("reported_import_head",
            "reported_export_head",
            "reported_import_usd", 
            "reported_export_usd",
            "difference_head",
            "difference_usd"))%>%
...
Imports <- edgelist %>%
  dplyr::select(receiver_iso3, year, mean) %>%
  rename("iso3" = receiver_iso3) %>%
  left_join(., global_historical_pop %>%
              dplyr::select(-Country) %>%
              rename("population" = Value,
                     "year" = Year),
            by = c("iso3", "year")) %>%
  group_by(iso3, year, population) %>%
  summarise(import = sum(mean)*1000) %>%
  filter(!(iso3 == "SVK" & year == 1999)) %>%
...
Exports <- edgelist %>%
  dplyr::select(sender_iso3, year, mean) %>%
  rename("iso3" = sender_iso3) %>%
  left_join(., global_historical_pop %>%
              dplyr::select(-Country) %>%
              rename("population" = Value,
                     "year" = Year),
            by = c("iso3", "year")) %>%
  group_by(iso3, year, population) %>%
  summarise(export = sum(mean)*1000) %>%
...
There are some discrepencies in the dataset between the amount of chickens reported by the exporting or importing countries. For this analysis I have taken the greatest number reported by either the receiver or sender and also the mean of the two. The below histograms show the result of this approach. I have explored the countries reporting more than 100,000 (1000 head) trading years but there are likely many more smaller errors in the data.
``` {r export_histogram}
...
detailed_trading %>%
  filter(difference_head < 100000 & difference_head > -100000) %>%
  ggplot(data = .) +
  geom_histogram(aes(x = difference_head)) +
  scale_y_sqrt() +
  labs(title = "Distribution of discrepencies in recording",
       x = "Difference between amount recorded by exporter and importer")
  
trade_difference <- detailed_trading %>%
  group_by(year, sender_iso3, receiver_iso3, difference_head) %>%
  summarise(trades = n()) %>%
  ungroup() %>%
  group_by(year)

all_trades <- count(no_diff, trades) %>%
  rename("all_trades" = n) %>%
  mutate(year = factor(year)) %>%
  select(year, all_trades)

no_diff <- trade_difference %>%
  filter(difference_head == 0)  %>%
  mutate(year = factor(year)) %>%
  group_by(year) %>%
  summarise(null_trades = n()) %>%
  left_join(., all_trades, by = "year") %>%
  pivot_longer(., 2:3, names_to = "trade", values_to = "values")

ggplot(data = no_diff, aes(x = year, y = values, fill = trade)) +
  geom_col() +
  labs(title = "Number of trades with no discrepency") +
  theme(axis.text.x = element_text(angle = 90))

small_diff <- detailed_trading %>%
  filter(difference_head < 1000 & difference_head > -1000) %>%
  mutate(year = factor(year))

ggplot(data = small_diff, aes(difference_head, colour = year)) +
  geom_freqpoly() +
  scale_y_sqrt() +
  labs(title = "Distribution of discrepencies in recording where less than 1000",
       x = "Difference between amount recorded by exporter and importer")

large_diff <- detailed_trading %>%
  filter(!difference_head < 1000 | !difference_head > -1000)
```
...
if(!file.exists(here("cleaned_data", "nD_graph.rds")))
  {
nodelist <- read_rds(here("cleaned_data", "nodelist_all.rds"))
data("World")
...
write_rds(nD_graph, here("cleaned_data", "nD_graph.rds"))
}

nD_graph <- read_rds(here("cleaned_data", "nD_graph.rds"))
```
...
exporters_1986 <- trade_map_1986 %>%
  group_by(sender_iso3) %>%
  summarise(export_value = sum(mean)) %>%
  filter(export_value > 1) %>%
  mutate(quartile = ntile(export_value, 3),
         quartile = factor(quartile, levels = c(1, 2, 3), labels = c("Low", "Medium", "High")))

importers_1986 <- trade_map_1986 %>%
  group_by(receiver_iso3) %>%
  summarise(import_value = sum(mean)) %>%
  filter(import_value > 1) %>%
  mutate(quartile = factor(
    ntile(import_value, 3),
    levels = c(1, 2, 3), labels = c("Low", "Medium", "High")))

net_exporters_1986 <- exporters_1986 %>%
  left_join(., importers_1986 %>%
              select(-quartile),
            by = c("sender_iso3" = "receiver_iso3" )) %>%
  mutate(import_value = ifelse(is.na(import_value), 0, import_value),
         net_export = export_value-import_value) %>%
  filter(net_export > 0) %>%
  select(-c(quartile, export_value, import_value)) %>%
  mutate(export_level = factor(
    ntile(net_export, 3),
    levels = c(1, 2, 3), labels = c("Export - low", "Export - medium", "Export - high")))

net_importers_1986 <- importers_1986 %>%
  left_join(., exporters_1986 %>%
              select(-quartile),
            by = c("receiver_iso3" = "sender_iso3" )) %>%
  mutate(export_value = ifelse(is.na(export_value), 0, export_value),
         net_import = import_value-export_value) %>%
  filter(net_import > 0) %>%
  select(-c(quartile, export_value, import_value)) %>%
  mutate(import_level = factor(
    ntile(net_import, 3),
    levels = c(1, 2, 3), labels = c("Import - low", "Import - medium", "Import - high")))

trade_factor <- union(levels(net_exporters_1986$export_level), levels(net_importers_1986$import_level))

nodelist_1986 <- c(trade_map_1986$sender_iso3, trade_map_1986$receiver_iso3) %>%
  unique() %>%
  as_tibble() %>%
  rename("node_iso3" = 1) %>%
  left_join(., wrld_simpl@data %>%
              select("ISO3", "LON", "LAT") %>%
              rename("node_iso3" = "ISO3",
                     "node_long" = "LON",
                     "node_lat" = "LAT"),
            by = "node_iso3") %>%
  left_join(., net_exporters_1986 %>%
              rename("trade" = export_level),
            by = c("node_iso3" = "sender_iso3")) %>%
  left_join(., net_importers_1986 %>%
              rename("trade" = import_level),
            by = c("node_iso3" = "receiver_iso3")) %>%
  mutate(trade = ifelse(is.na(trade.x),
                        as.character(trade.y),
                        as.character(trade.x)),
         trade = ifelse(is.na(trade), "NA", trade),
         trade = factor(trade, levels = trade_factor)) %>%
  select(node_iso3, node_long, node_lat, trade)

node_bounds <- nodelist_1986 %>%
  select(node_long, node_lat) %>%
  drop_na %>%
...
ggplot() +
  geom_polygon(data = map_data(map = "world") %>%
               ...
               alpha = 0.25,
               colour = "white") +
  geom_segment(data = trade_map_1986,
               aes(x = sender_long, xend = receiver_long,
                   y = sender_lat, yend = receiver_lat),
               size = 0.05,
               alpha = 0.5) +
  geom_point(data = nodelist_1986,
             aes(x = node_long,
                 y = node_lat,
                 colour = trade)) +
  theme_bw() +
  theme(axis.line = element_line(),
        ...
        axis.text = element_blank()) +
  coord_map("polyconic",
            xlim = c(node_bounds["node_long", "min"],
                     node_bounds["node_long", "max"]),
            ylim = c(node_bounds["node_lat", "min"],
                     node_bounds["node_lat", "max"]))
```
